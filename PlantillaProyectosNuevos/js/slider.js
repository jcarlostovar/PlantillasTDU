...
(function ($) {
    $.fn.slider = function () {
        var settings = $.extend({
            url: "https://tduwebcomercios.azurewebsites.net/api/sliderweb/1/6"
        });
        ...
        var init = function () {
            $.ajax({
                ...
                render({sliders: response});
            });
        };

        var render = function (response) {
            var html = new EJS({url: 'js/slider.ejs'}).render(response);
            $("#cont-items").html(html);
            loadSlide();
        }
